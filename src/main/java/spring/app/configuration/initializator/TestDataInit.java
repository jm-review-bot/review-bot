    ...
    private void init() throws Exception {
        ...
        if (propertyDB.equalsIgnoreCase("create") ||
            ...
            * Общие для них данные создаются до проверки условия, с каким конфигурационным файлом запущено приложение */
            ...
            /* Выполняется проверка, с каким конфигурационным файлом было запущено приложение:
            *
            * devops-app.properties - для сервера,
            * local-app.properties - для ПК разработчика/тестировщика */
            MutablePropertySources propertySources = ((ConfigurableEnvironment) environment).getPropertySources();
            if (propertySources.contains("class path resource [local-app.properties]")) {
                ...
                User ulvi = new User();
                ulvi.setFirstName("Ulvi");
                ulvi.setLastName("Rustamzade");
                ulvi.setReviewPoint(10);
                ulvi.setVkId(603905976);
                ulvi.setRole(roleAdmin);
                ulvi.setChatStep(StepSelector.START);
                userService.addUser(ulvi);
                ...
                freeTheme1.setExaminers(Arrays.asList(defaultUser, mikhail, nikolay, slyab));
